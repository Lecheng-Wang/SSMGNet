...
# @function: Predict large size(H*W) MSI
...
def raster_tif_to_shp(tif_path, shp_dir, ignore_class=0, epsg_code=None):
    ...
    if len(unique_values) == 0:
        print("No valid classes found in the prediction.", flush=True)
        return
    ...
    if len(all_geometries) == 0:
        print("No geometries extracted.", flush=True)
        return
    ...
    print(f"Shapefile saved in: {shp_path}, total objects: {len(gdf)}", flush=True)
...
def save_prediction_as_geotiff(prediction, geotransform, projection, output_path):
    ...
    dataset.FlushCache()
    dataset = None
    print(f"Prediction already saved with GeoTIFF in : {output_path}", flush=True)
...
def main(model_cfg, model_path, input_image_path, output_tiff_path, output_png_path, shape_out_dir):
    ...
    try:
        ...
        print(f"Image dimension: {image.shape[1]}x{image.shape[2]}, bands num: {image.shape[0]}", flush=True)
    except Exception as e:
        print(f"Faile to read image: {e}", flush=True)
        return

    print("Prediction on going....", flush=True)
    try:
        predicted_result     = model.predict_large_image(image)
        predicted_png_result = model.get_large_predict_png(image)
    except Exception as e:
        print(f"Faile to predict image: {e}", flush=True)
        return
    ...
    try:
        ...
        print(f"Faile to save prediction: {e}", flush=True)
        return
    
    try:
        raster_tif_to_shp(tif_path=output_tiff_path, shp_dir=shape_out_dir)
    except Exception as e:
        print(f"Failed to transfer into shapefile: {e}", flush=True)
        return
