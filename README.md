...
This repository implements SSMGNet (Spectral Similarity Measurement Guided Network) for semantic segmentation (example use: glacier extraction).  
It combines a segmentation network (SegModel) with an attention supervision signal computed from spectral sample references. The attention supervision is generated by `utils/sf_cnm_threshold.py` and used during training to improve model focus on spectrally-similar pixels.
...
## Contents (high level)
- train.py — main training script (single / multi / all-GPU modes)
- predict_small.py — inference for small patches (includes Grad-CAM and attention heatmaps)
- predict_huge.py — inference for full large images + raster → shapefile conversion
- utils/sf_cnm_threshold.py — compute misclassification thresholds and attention supervision labels
- model/..., utils/... — model, dataset, evaluation utilities
...
- PyTorch >= 1.12 (install a build matching your CUDA)
- numpy
- tqdm
- GDAL (gdal)
- rasterio
...
Example (conda, minimal):
```bash
conda create -n ssmgnet python=3.9 -y
conda activate ssmgnet

# install torch appropriate for your CUDA (example; change according to your GPU)
pip install torch torchvision --extra-index-url https://download.pytorch.org/whl/cu118

pip install numpy tqdm gdal rasterio geopandas opencv-python matplotlib
```
...
If you want, I can:
- Convert predict scripts to accept CLI arguments (argparse) for easier use.
- Extract default hyperparameters from `train.py` and generate a ready-to-edit YAML config file for experiments.
- Generate a short troubleshooting checklist tailored to your dataset and GPU setup.
